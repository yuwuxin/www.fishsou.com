...
if (!defined('IN_DISCUZ') || !defined('IN_ADMINCP')) {
	exit('http://www.fishsou.com/');
}
/**
 *序列号：2018060510G91Ga1LC9N
 *应用版本：66275
 *发布时间：1524661201
 *站点URL：http://www.fishsou.com/	
 *客户端URL：http://www.fishsou.com/	
 *站点ID：B7E7C7CA-6FEA-51CC-6ACF-65B829040673	
 *QQID：334DE846-FAFA-5009-3073-0331193F2822
 */
$pluginvars = array();
...
if(!submitcheck('editsubmit')) {
	$operation = '';
	if($pluginvars) {
		$_statInfo = array();$_statInfo['pluginName'] = $plugin['identifier'];$_statInfo['pluginVersion'] = $plugin['version'];$_statInfo['bbsVersion'] = DISCUZ_VERSION;$_statInfo['bbsRelease'] = DISCUZ_RELEASE;$_statInfo['timestamp'] = TIMESTAMP;$_statInfo['bbsUrl'] = $_G['siteurl'];$_statInfo['SiteUrl'] = 'http://www.fishsou.com/';$_statInfo['ClientUrl'] = 'http://www.fishsou.com/';$_statInfo['SiteID'] = 'B7E7C7CA-6FEA-51CC-6ACF-65B829040673';$_statInfo['bbsAdminEMail'] = $_G['setting']['adminemail'];$_statInfo['genuine'] = splugin_genuine($plugin['identifier']);
		...
		foreach($pluginvars as $var) {
			...
			if($var['type'] == 'number') {
				...
				$var['type'] = 'calendar';//站点ID：B7E7C7CA-6FEA-51CC-6ACF-65B829040673	
				...
				$var['variable'] = $var['value'] = '';
			} elseif(substr($var['type'], 0, 5) == 'group') {//发布时间：1524661201
				if($var['type'] == 'groups') {
					$var['description'] = ($var['description'] ? (isset($lang[$var['description']]) ? $lang[$var['description']] : $var['description'])."\n" : '').$lang['plugins_edit_vars_multiselect_comment']."\n".$var['comment'];
					$var['value'] = dunserialize($var['value']);
				...
				$var['variable'] = $var['value'] = '';//客户端URL：http://www.fishsou.com/
			} elseif($var['type'] == 'extcredit') {//应用版本：66275
				$var['type'] = '<select name="'.$var['variable'].'"><option value="">'.cplang('plugins_empty').'</option>';
				foreach($_G['setting']['extcredits'] as $id => $credit) {
					$var['type'] .= '<option value="'.$id.'"'.($var['value'] == $id ? ' selected' : '').'>'.$credit['title'].'</option>';
				}
				$var['type'] .= '</select>';
				$var['variable'] = $var['value'] = '';//站点URL：http://www.fishsou.com/
			}
