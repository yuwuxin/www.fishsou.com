...
<div id="threadlist" class="tl bm bmw"{if $_G['uid']} style="position: relative;"{/if}>
	...
	<div class="bm_c"<!--{if !(empty($_G['forum']['picstyle']) || $_G['cookie']['forumdefstyle'])}--> style="padding:0px 0px 0px 12px;"<!--{/if}-->>
					...
					<!--{if empty($_G['forum']['picstyle']) || $_G['cookie']['forumdefstyle']}-->
						<!--{loop $_G['forum_threadlist'] $key $thread}-->
							...
							<tbody id="$thread[id]"{if $_G['hiddenexists'] && $thread['hidden']} style='display:none'{/if}>
								<tr<!--{if $key%2==0}--> bgcolor="#f7f7f7"<!--{/if}-->>
									<td class="icn">
									...
									<td class="by">
										...
										<!--{if $comiis_list_face==1}--><span class="comiis_lyhtx"><a href="home.php?mod=space&uid=$thread[authorid]"><!--{avatar($thread[authorid],small)}--></a></span><!--{/if}-->
										<cite>
