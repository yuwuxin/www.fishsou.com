...
<div id="postlist" class="pl bm thfd comiis_x0">
	<table cellspacing="0" cellpadding="0" class="comiis_viewtop">
		<tr>
			...
			<td class="pls ptn pbn">
				...
				<!--{else}-->
				<div class="hm ptn">
					<span>{lang show}:</span> <span>$_G[forum_thread][views]</span><span class="pipe">|</span><span>{lang reply}:</span> <span>$_G[forum_thread][allreplies]</span>
				</div>
				<!--{/if}-->
			</td>
			<!--{/if}-->
	...
	<!--{loop $postlist $post}-->
		<!--{if $rushreply && $_GET['checkrush'] && $post['rewardfloor'] != 1}-->
			<!--{eval continue;}-->
		<!--{/if}-->
		<div id="post_$post[pid]" {if $_G['blockedpids'] && $post['inblacklist']}style="display:none;"{/if} class="comiis_vrx"<!--{if $postcount==0}--> style="border-radius:0px 0px 4px 4px;"<!--{/if}-->>
