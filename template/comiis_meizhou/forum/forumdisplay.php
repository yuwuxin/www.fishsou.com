...
<DIV class="comiis_l190">
  <div class="list_left"<!--{if $comiis_list_zlb_tx==0}--> style="padding-top:7px;"<!--{/if}-->>
	<!--{if $comiis_list_zlb_tx==1}-->
    <div class="widget left_nav" style="border-top:0px solid #DDDDDD;">
      <ul style="margin:0 8px 10px;">
        <li class="km_icon icon_top"><a href="forum.php?mod=forumdisplay&fid=$_G['cache']['forums'][$_G['forum']['fup']]['fid']">$_G['cache']['forums'][$_G['forum']['fup']]['name']</a></li>
        <!--{eval $comiis_nn=0;}-->
        <!--{loop $_G['cache']['forums'] $comiis_f}-->
			<!--{if $comiis_f['status'] && $comiis_f['fup'] == $_G['forum']['fup']}-->
				<!--{eval $comiis_nn++;}-->
				<li{if $comiis_f['fid']==$_G['forum'][fid]} class="select"{/if}><a href="forum.php?mod=forumdisplay&fid={$comiis_f['fid']}">{$comiis_nn}F:{$comiis_f['name']}</a></li>
			<!--{/if}-->
        <!--{/loop}-->
      </ul>
    </div>
